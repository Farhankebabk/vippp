-- V1PRWARE - FORSAKEN COMPLETE EDITION
-- Original by V1pr & Mitsuki | Addons: Instant Generator Fix, Body ESP, Auto Teleport Generator
print("v1prware loading with all addons...")
...
do
    ...
    end    local function destroyEmoteGui()
        emStop()
...
-- ============================================================
-- INSTANT GENERATOR (PERBAIKAN – FORCE COMPLETE)
-- ============================================================
do -- tabGen (Instant Mode - FIXED)
    ...
    -- === FUNGSI INSTANT SOLVE YANG LEBIH AGRESIF ===
    local function instantFlowSolve(puzzle)
        if not puzzle then
            warn("[Instant] Puzzle is nil")
            return false
        end

        if not puzzle.Solution or #puzzle.Solution == 0 then
            warn("[Instant] Puzzle has no solution")
            return false
        end
        ...
        -- Paksa cek kemenangan
        if solved then
            pcall(function() puzzle:checkForWin() end)
            print("[Instant] Puzzle force-completed!")
        else
            warn("[Instant] No solution paths found")
        end
    ...
    local function hookInstantSolve()
        pcall(function()
            ...
            if not fgModule then
                warn("[Instant] FlowGame module not found")
                return
            end

            local ok, FG = pcall(require, fgModule)
            if not ok or not FG then
                warn("[Instant] Failed to require FlowGame")
                return
            end

            if not FG.new then
                warn("[Instant] FG.new not found")
                return
            end
            ...
            FG.new = function(...)
                ...
                if instant.on then
                    -- Tunggu puzzle siap, lalu selesaikan
                    task.spawn(function()
                        -- Beri waktu inisialisasi (0.1-0.3 detik)
                        task.wait(0.1)
                        local success = instantFlowSolve(puzzle)
                        if not success then
                            -- Coba lagi setelah 0.5 detik
                            task.wait(0.5)
            ...
            print("[Instant] Hook applied successfully")
        end)
    end
    ...
    secInstant:Toggle({
        ...
        Callback = function(on)
            pcall(function()
                ...
                if on then
                    hookInstantSolve()
                    print("[Instant] Mode ENABLED")
                else
                    print("[Instant] Mode DISABLED")
                end
            end)
        end
    })

    -- === FORCE COMPLETE (TOMbol) ===
    secInstant:Button({
        Title = "Force Complete Now",
        Callback = function()
            pcall(function()
                -- Cari puzzle aktif secara manual
                local modFolder = svc.RS:FindFirstChild("Modules")
                local miniFolder = modFolder and modFolder:FindFirstChild("Minigames")
                local fgFolder = miniFolder and miniFolder:FindFirstChild("FlowGameManager")
                if fgFolder then
                    for _, child in ipairs(fgFolder:GetChildren()) do
                        if child:IsA("ModuleScript") and child.Name == "FlowGame" then
                            ...
                            if ok and FG and FG.getActivePuzzle then
                                local active = FG.getActivePuzzle()
                                if active then
                                    instantFlowSolve(active)
                                    print("[Instant] Force complete triggered")
                                    break
                                end
                            end
                        end
    ...
    -- Jalankan hook sekali di awal
    hookInstantSolve()
    print("[v1prware] Instant Generator (Fixed) loaded")
end -- end Instant Mode (FIXED)

-- ============================================================
-- ESP BENTUK TUBUH (OUTLINE) – DI TAB GENERATOR
-- ============================================================
do -- tabGen (ESP Section)
    local secESP = tabGen:Section({ Title = "ESP Player", Opened = true })
    ...
    -- Warna
    local COLOR_KILLER = Color3.fromRGB(255, 0, 0)   -- Merah
    local COLOR_SURVIVOR = Color3.fromRGB(0, 255, 0) -- Hijau

    -- Fungsi untuk membuat outline (Highlight) pada karakter
    local function applyHighlight(character, isKiller)
        if not character then return end
        -- Hapus highlight lama jika ada
        local oldHighlight = character:FindFirstChild("ESP_Highlight")
        if oldHighlight then oldHighlight:Destroy() end

        local color = isKiller and COLOR_KILLER or COLOR_SURVIVOR

        -- Buat Highlight (outline) – non-intrusive
        local highlight = Instance.new("Highlight")
        highlight.Name = "ESP_Highlight"
        highlight.Adornee = character
        highlight.FillColor = color
        highlight.FillTransparency = 0.5  -- Semi-transparan agar tidak mengganggu
    ...
    local function clearESP()
        ...
        -- Hapus semua Highlight di semua karakter
    ...
    -- Refresh ESP (aplikasikan ke semua pemain)
    local function refreshESP()
        ...
        if killerFolder then
            local conn = killerFolder.ChildAdded:Connect(onCharacterAdded)
            table.insert(esp.connections, conn)
        end
        if survivorFolder then
            local conn = survivorFolder.ChildAdded:Connect(onCharacterAdded)
            table.insert(esp.connections, conn)
        end

        -- Refresh saat karakter kita berganti
    ...
    -- ============================================================
    -- UI CONTROLS DI TAB GENERATOR
    -- ============================================================
    secESP:Toggle({
        Title = "ESP Bentuk Tubuh (Killer Merah, Survivor Hijau)",
        Type = "Checkbox",
        Flag = "espBody",
        Default = esp.on,
        Callback = function(on)
    ...
    -- Slider untuk mengatur transparansi fill (agar lebih tidak mengganggu)
    local fillTrans = 0.5
    secESP:Slider({
        Title = "Fill Transparency",
        Flag = "espFillTrans",
        Step = 0.05,
        Value = { Min = 0, Max = 1, Default = fillTrans },
        Callback = function(v)
            fillTrans = v
    ...
    print("[v1prware] ESP Body Outline loaded in Generator tab")
end -- end tabGen (ESP Section)

-- ============================================================
-- AUTO TELEPORT TO GENERATOR (FORSAKEN)
-- ============================================================
do -- tabGen (Teleport Generator)
    local secTeleport = tabGen:Section({ Title = "Generator Teleport", Opened = true })
    ...
    -- Fungsi untuk mencari semua generator di map
    local function findGenerators()
        local gens = {}
        local mapContent = getMapContent()
        if not mapContent then return gens end

        -- Cari objek dengan nama yang mengandung kata kunci
        for _, obj in ipairs(mapContent:GetDescendants()) do
            if obj:IsA("BasePart") or obj:IsA("Model") then
                local name = obj.Name:lower()
                if name:match("generator") or name:match("gen") or name:match("power") or name:match("node") then
                    -- Ambil posisi (jika BasePart) atau cari PrimaryPart/HRP
                    local pos = nil
                    if obj:IsA("BasePart") then
                        pos = obj.Position
                    elseif obj:IsA("Model") then
                        local prim = obj:FindFirstChild("PrimaryPart") or obj:FindFirstChild("HumanoidRootPart")
    ...
    -- Fungsi teleportasi ke posisi tertentu
    local function teleportTo(position)
        local char = lp.Character
        if not char then return false end
        local root = char:FindFirstChild("HumanoidRootPart") or char:FindFirstChild("PrimaryPart")
        if not root then return false end

        -- Teleport dengan CFrame
        root.CFrame = CFrame.new(position + Vector3.new(0, 3, 0)) -- Offset agar tidak terjebak
        return true
    end

    -- Teleport ke generator terdekat
    local function teleportToNearestGen()
        local gens = findGenerators()
        if #gens == 0 then
            print("[Teleport] No generators found!")
            return false
        end
        ...
        for _, gen in ipairs(gens) do
            local dist = (gen.position - myPos).Magnitude
        ...
        if nearest then
            ...
            print("[Teleport] Teleported to nearest generator: " .. nearest.name)
            return true
        end
        return false
    end

    -- Teleport ke generator berikutnya (siklus)
    local genIndex = 1
    local function teleportToNextGen()
        local gens = findGenerators()
        if #gens == 0 then
            print("[Teleport] No generators found!")
            return false
        ...
        local target = gens[genIndex]
        if target then
            teleportTo(target.position)
            print("[Teleport] Teleported to generator " .. genIndex .. ": " .. target.name)
            genIndex = genIndex + 1
            return true
        end
        return false
    ...
    -- Auto-teleport loop (berpindah antar generator setiap X detik)
    local function startAutoTeleport(interval)
        if tele.loop then
            task.cancel(tele.loop)
            tele.loop = nil
        end

        if not tele.auto then return end

        tele.loop = task.spawn(function()
            while tele.auto do
                local success = teleportToNextGen()
                if not success then
                    -- Jika gagal, cari ulang generator
                    tele.genList = findGenerators()
                    if #tele.genList == 0 then
                        print("[Teleport] No generators found, retrying in 5s...")
                        task.wait(5)
                    end
                end
                task.wait(interval)
    ...
    secTeleport:Button({
        ...
        Callback = function()
            pcall(teleportToNearestGen)
        end
    })

    secTeleport:Button({
        Title = "Teleport to Next Generator",
        Callback = function()
            pcall(teleportToNextGen)
        end
    })

    secTeleport:Toggle({
        Title = "Auto Teleport (Cycle Generators)",
        Type = "Checkbox",
        Flag = "autoTeleGen",
        Default = tele.auto,
        Callback = function(on)
            tele.auto = on
            if on then
                -- Mulai auto dengan interval default 5 detik
                startAutoTeleport(5)
            else
                if tele.loop then
                    task.cancel(tele.loop)
    ...
    -- Slider untuk interval auto-teleport
    local autoInterval = 5
    secTeleport:Slider({
        Title = "Auto Teleport Interval (s)",
        Flag = "teleInterval",
        Step = 1,
        Value = { Min = 1, Max = 20, Default = autoInterval },
        Callback = function(v)
            autoInterval = v
            if tele.auto then
                -- Restart auto dengan interval baru
                startAutoTeleport(autoInterval)
            end
        end
    })

    -- Tombol refresh daftar generator
    secTeleport:Button({
        Title = "Refresh Generator List",
        Callback = function()
            tele.genList = findGenerators()
            print("[Teleport] Found " .. #tele.genList .. " generators")
            for i, gen in ipairs(tele.genList) do
                print("  " .. i .. ". " .. gen.name .. " at " .. tostring(gen.position))
            end
        end
    })

    print("[v1prware] Generator Teleport loaded")
end -- end tabGen (Teleport Generator)
...
print("[v1prware] All tabs loaded successfully with all addons!")
